...
def create_api_app(
    ...
    if endpoints is None:
        endpoints = default_decoy_endpoints()
...
def _make_handler(
    ...
    async def handler(request: Request) -> JSONResponse:
        ...
        event = TriggerEvent(
            ...
            canary_id=f"api_endpoint:{endpoint.path}",
        ...
        logger.warning(
            "API decoy triggered: %s %s from=%s event_id=%s",
            request.method,
            endpoint.path,
        ...
        return JSONResponse(endpoint.response_body, status_code=endpoint.response_status)
